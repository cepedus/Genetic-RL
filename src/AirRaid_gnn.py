...
class AirRaidGNN(GeneticNeuralNetwork):
    pass
...
if __name__ == '__main__':
    ...
    obs = env.reset()
    print(obs.shape)
    layers_shapes = [obs.shape[0], obs.shape[0], int(obs.shape[0]/2), env.action_space.n]  # Format of the neural network
    print(layers_shapes)
    dropout_rate = 0.1   # Chance of dropout
    baseline_fitness = 1000   # Minimum baseline for the optimized random initialization

    # Folder name for good ol' Windows
    dirname = os.path.dirname(__file__)
    out_folder = filename = os.path.join(dirname, '../models/airraid/')

    initial_network = AirRaidGNN(layers_shapes, dropout=dropout_rate)   # Instantiate
